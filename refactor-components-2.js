...
function replaceInFile(filePath, replacements) {
  ...
  for (const [oldStr, newStr] of replacements) {
    ...
    let escapeStr = oldStr.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
    content = content.replace(new RegExp('"' + escapeStr + '"', 'g'), newStr);
    content = content.replace(new RegExp("'" + escapeStr + "'", 'g'), newStr);
    
    // 2. match JSX literal text (between > and <)
    const jsxRegex = new RegExp('>\\s*' + escapeStr + '\\s*<', 'g');
    content = content.replace(jsxRegex, '>{' + newStr + '}<');
  }
...
replaceInFile('./src/app/(main)/kho/components/ban-thanh-pham.tsx', [
  ['Tổng Bán thành phẩm', 't("dashboard.semiProduct", "Tổng Bán thành phẩm")'],
...
console.log("Done phase 2 refactoring.");
